...
int rb_remove(struct rb_tree *tree, struct rb_node *node)
{
	...
	while (node->parent) {
		...
		if (!sibling->child[dir] || sibling->child[dir]->color ==
			RB_BLACK) {
			sibling->color = RB_RED;
			/* Verify sibling->child[!dir] exists before changing its color */
			if (sibling->child[!dir]) {
				sibling->child[!dir]->color = RB_BLACK;
			}
			
			rotate_node(tree, node->parent, dir);
			
			/* After rotation, check if node->parent is still valid */
			if (!node->parent) break;

			sibling = node->parent->child[dir];
			if (!sibling) 
				break; /* Sibling is NULL post rotation*/
