...
{
	return dump_page_tables_range(pml4, mask, 0, (void *) KERNEL_LIM);
}

/* Given the root pml4 to the page table hierarchy, dumps the mapped regions
 * with the same flags in the given range. mask can be PAGE_HUGE to differentiate
 * regions mapped with normal pages from those mapped with huge pages.
 */
